...
export class Msg {
  static alert(msg: string, func = () => {}, title = "") {
    ElMessageBox.close();
...
export class Tools {
  ...
  static uniqueId() {
    return (
      ...
      Number(
        "".concat(Date.now().toString()).concat((++id$2).toString()),
      ).toString(36)
    );
  }
  ...
  static getCurrentTimeRange(timeType: number) {
    let start, end;
    if (timeType === 0) {
      //当前时间区间为前一天的21:00到今天的15:00, 如果是周一的话,向前推3天,到上周五 ALGOQI-629
      start = dayjs()
        .subtract(dayjs().day() === 1 ? 3 : 1, "day")
        .hour(20)
        .minute(0)
        .second(0);
