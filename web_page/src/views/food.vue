            ...
            <el-scrollbar class="flex flex-col h-full">
                <el-checkbox-group v-model="selectItems">
                    <div
                        ...
                        :key="i"
                        class="flex justify-between food-item mb-3 pb-2 text-sm"
                    >
                        <div v-if="isDialog" class="w-10">
                            <el-checkbox :value="item" />
                        </div>
                        <div
                            class="flex-1 flex justify-start pl-4 items-center hand"
                            @click="checkFood(item)"
                        >
                            {{ item.key }}
                        </div>
                        <div
                            class="flex-1 flex justify-center items-center hand"
                            @click="checkFood(item)"
                        >
                            {{ item.val }}
                        </div>
...
function edit(item) {
    ...
    state.show = true;
    state.curIndex = state.list.findIndex(
        (item) => item.key === state.curFood.key,
    );
}
...
function update() {
    formRef.value.validate(async (valid: boolean) => {
        if (valid) {
            ...
            if (
                list.findIndex((item) => item.key === state.curFood.key) !== -1
            ) {
