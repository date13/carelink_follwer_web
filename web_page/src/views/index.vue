    ...
    <MainPanel no-pad="1">
        <div class="flex flex-col h-full bg-white overflow-x-hidden pa-1">
            ...
            <div class="flex flex-row h-45">
                <div class="w-1/2 flex items-center justify-center">
                    <el-card class="w-max info-panel ma-1 max-w-110">
                        <template #header>
                            <div
                                class="card-header text-center flex items-center justify-between"
                            >
                                <span
                                    :class="{ 'text-red': status !== 200 }"
                                    class="text-xs hand"
                                    @click="reloadPage"
                                    >状态:{{ status }}</span
                                >
                                <span
                                    class="text-2xl font-bold hand"
                                    @click="reloadCarelinkData"
                        ...
                        <div class="flex flex-wrap">
                            ...
                            <el-tag
                                class="mb-1 mr-1 only-full-height"
                                size="small"
                                type="primary"
                            >
                                <span class="text-red"
                                    >Wav:
                                    {{
                                        sugarCalc.maxWave(data.sgs, setting)
                                    }}</span
                                >&nbsp;
                                <span class="text-red"
                                    >Min: {{ minMaxSG[0] }}</span
                            ...
                            >
                                ...
                                {{ tightTimeInRange[0] }}%
                                <span class="text-rose mx-1"
                                    >L: {{ tightTimeInRange[1] }}%
                                </span>
                                <span class="text-rose"
                                    >H: {{ tightTimeInRange[2] }}%
                                </span>
                            </el-tag>
                            <el-tag
                                v-if="
                            ...
                            >
                                <div
                                    v-for="{ messageId, sg } in data
                                        .notificationHistory
                                        .activeNotifications"
                                >
                                    {{
                                        NOTIFICATION_MAP[messageId]
                                            ? sugarCalc.showNotificationMsg(
                                                  NOTIFICATION_MAP[messageId],
                                                  sg,
                                              )
                                            : messageId
                ...
                <div class="w-1/2 flex items-center justify-center flex-col">
                    <div class="flex items-baseline">
                        ...
                        <div v-if="lastNSData" class="flex items-center">
                            ...
                            <Trend
                                :small="true"
                                :trend-obj="DIRECTIONS[lastNSData.direction]"
                            ></Trend>
                        </div>
                    </div>
                    <div
                        class="flex text-xs items-center justify-between align-center"
                    >
                        <span
                            :class="{ 'text-red': lastUpdateTime.sgDiff >= 15 }"
                            class="mx-2"
                        >
                            {{ lastUpdateTime.sg }}
                        </span>
                    ...
                    <div
                        class="flex items-center justify-center time-range mt-1 only-full-height"
                    >
                        <el-radio-group
                            v-model="setting.startPercent"
                            size="small"
                            @change="refreshChart"
                        >
                            <el-radio-button
                                v-for="item in TIME_RANGE_CONFIG"
                                :label="item.label"
                                :value="item.value"
                            />
                        </el-radio-group>
                    ...
                    <div
                        class="flex text-xs items-center justify-between align-center my-1"
                    >
                        <span
                            v-if="
                                data.systemStatusMessage ===
                                SYSTEM_STATUS_MAP.WARM_UP.key
                            "
                            class="mx-2"
                        >
                            <el-popover
                            ...
                            >
                                <template #reference>
                                    预计启动:&nbsp;{{ nextStartTimeToNow }}
                                </template>
                            </el-popover>
                        </span>
                        <span v-else class="mx-2"
                            >更新:&nbsp;{{ updateDatetime }}</span
                        >
                        <div
                            :style="{
                                color: SYSTEM_STATUS_MAP[
                                    data.systemStatusMessage
                                ]?.color,
                            }"
                            class="mr-2"
                        >
                            {{
                                SYSTEM_STATUS_MAP[data.systemStatusMessage]
                                    ?.name
                            }}
                        </div>
                    </div>
                    <div
                        class="flex items-center justify-around align-center info-panel only-full-height"
                    >
                        <Device
                            :data="{
                                ...
                                sensorLastDatetime: sugarCalc.sensorState(
                                    data,
                                    false,
                                ),
            ...
            <div class="flex-1 chart-panel flex">
                <div
                    ref="myChart"
                    class="flex-1 border-grey border-grey h-full only-full-height"
                ></div>
            ...
            <div class="h-10 px-2 flex items-center justify-around">
                ...
                <Conduit
                    ...
                    @updateConduitDatetime="updateConduitTime"
                ></Conduit>
                <el-tag size="large" type="primary">
                    <div class="flex flex-col">
                        <span
                            >剂量(昨): {{ lastUpdateTime.sumInsulin || "--" }}U
                        </span>
                        <span
                            >基础(昨):
        ...
        <div class="float-panel flex flex-col items-center justify-center">
            ...
            <div
                :class="{ 'no-top': showSetting }"
                class="item flex items-center justify-center border-solid border-1 hand"
                @click="reload"
            >
        ...
        ></NotificationDialog>
        <BasalDialog
            v-if="showBasalDialog"
            v-model:show="showBasalDialog"
            :basals="
                data.markers.filter(
                    (item) =>
                        item.type === INSULIN_TYPE.AUTO_BASAL_DELIVERY.key,
                )
            "
        ></BasalDialog>
    </MainPanel>
...
const { drawer, showSetting } = toRefs(state);
...
function switchAR2() {
    if (!sugarCommon.state.data?.lastSG) return;
    if (!sugarCalc.shouldHaveAR2(sugarCommon.state.data) && setting.showAR2) {
        setting.showAR2 = false;
        Msg.warnMsg(
            `当前系统状态为:${SYSTEM_STATUS_MAP[sugarCommon.state.data.systemStatusMessage]?.name},最近血糖为:${SG_STATUS[sugarCommon.state.data.lastSG.sensorState]?.name}`,
        );
    } else {
        refreshChart();
    }
...
function refreshChart() {
    ...
    if (!chart) drawLine();
    chart.setOption(charOption.value, { notMerge: true, lazyUpdate: true });
}
    ...
    return {
        legend: {
            ...
            data: is_nsData
                ? [
                      ...CHART_LEGEND,
                      { name: "NS血糖", itemStyle: { color: COLORS[2] } },
                  ]
                : CHART_LEGEND,
        ...
        tooltip: {
          ...
          <span>${dayjs(param.data[0]).format("HH:mm")}</span>
                ...
                params.forEach((item, i) => {
                    ...
                    const isInsulin =
                        type.key === INSULIN_TYPE.INSULIN.key ||
                        type.key === INSULIN_TYPE.INSULIN_YESTERDAY.key;
                    dataStr += `
        ...
        yAxis: [
            ...
            },
            {
                ...
                min: 0,
                max:
                    sugarCalc.loadBaselData(sugarCommon.state.data.markers)
                        .max + 1,
            },
            {
                name: "大剂量",
        ...
        series: [
            {
                name: "血糖",
                type: "line",
                data: sugarCalc.loadSgData(
                    sugarCommon.state.data,
                    state.forecast.ar2,
                    setting,
                ),
                smooth: true,
            ...
            {
                name: "校准",
                data: sugarCalc.loadCalibrationData(
                    sugarCommon.state.data.markers,
                ),
                type: "scatter",
                yAxisIndex: 1,
                symbol: (value, params) => {
            ...
            },
            {
                ...
                label: {
                    ...
                    color: "inherit",
                    formatter: (item) => {
                        return item.data[2].key ===
                            INSULIN_TYPE.AUTOCORRECTION.key
                            ? item.data[1]
                            : "";
                    },
                    position: "bottom",
            ...
            },
            {
                ...
                markLine: {
                    ...
                    label: {
                        ...
                        formatter: (item) => {
                            return item.data.last
                                ? `${dayjs(item.value).format("HH:mm")}${item.data.type === "start" ? "\n" + item.data.data.delivered : ""}`
                                : "";
                        },
                    },
                    emphasis: {
                    ...
                    data: sugarCalc.showInsulinPeak(
                        sugarCommon.state.data.markers,
                        setting,
                    ),
                },
                data: sugarCalc.loadInsulinData(
                    sugarCommon.state.data.markers,
                    setting,
                ),
            },
            {
                name: "血糖(昨)",
                type: "line",
                data: sugarCalc.loadYesterdaySgData(
                    sugarCommon.state.myData.yesterday,
                    setting,
                ),
                smooth: true,
                connectNulls: false,
            ...
            },
            {
                ...
                data: sugarCalc.loadInsulinData(
                    sugarCommon.state.myData.yesterday.markers,
                    setting,
                    INSULIN_TYPE.INSULIN_YESTERDAY,
                ),
            },
            is_nsData
                ? {
                      name: "NS血糖",
                      type: "line",
                      data: sugarCalc.loadNsSgData(
                          sugarCommon.state.nsData,
                          setting,
                      ),
                      smooth: true,
                      connectNulls: false,
