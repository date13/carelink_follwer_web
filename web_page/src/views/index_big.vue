        ...
        <div class="flex h-full bg-black overflow-x-hidden text-white p-4">
            ...
            <div class="flex flex-col w-3/5 big-panel mr-4">
                <div class="h-10 flex w-full justify-between items-center px-4">
                    ...
                    <div v-if="lastNSData" class="flex items-center">
                        ...
                        >
                            {{
                                Tools.toNow(
                                    sugarCalc.cleanTime(lastNSData.datetime),
                                )
                            }}
                        </div>
                        <div
                ...
                <div class="flex flex-col w-full h-full flex-1">
                    ...
                    <div
                        class="h-20 flex items-center justify-center text-base"
                    >
            ...
            <div class="flex chart-panel flex-col w-2/5 h-full">
                <div class="flex-1 flex flex-col">
                    ...
                    <div
                        class="flex w-full h-2/5 border border-solid border-zinc-300 p-2"
                    >
                        <div
                            v-for="(item, i) in statistics"
                            :class="{
                                'border-r-zinc border-r border-r-solid':
                                    i === 'day30',
                            }"
                            class="flex w-1/2 flex-col h-full justify-around items-center"
                        >
                            <div
                                class="text-lg font-bold h-10 flex items-center"
                            >
                                {{ i === "day30" ? 30 : 90 }}日
                            </div>
                    ...
                    <el-tag size="large" type="primary">
                        <div class="flex flex-col">
                            ...
                            <span
                                ...
                                {{
                                    userSetting.ns
                                        ? userSetting.manual_basal.toFixed(2)
                ...
                >
                    ...
                    >
                        <div class="float-item">
                            <el-checkbox
                                v-model="setting.notification.alarmEnable"
                                label="报警"
                                size="small"
                            />
                        </div>
...
const { statistics, showLogsDialog, showSetting, userSetting } = toRefs(state);
...
function alarmNotification(
    ...
    if (notifyObj && notifyObj.alarm && !state.playAlarmObj.playing) {
        ...
        if (
            !notification.lastAlarm.key ||
            instanceId !== notification.lastAlarm.key ||
            isForce
        ) {
...
function playAlarm(type = "error") {
    ...
    if (
        playAlarmObj.count <= playAlarmObj.totalPlayCount &&
        !playAlarmObj.playing
    ) {
        ...
        playAlarmObj.alarmAudio
            ...
            .then((res) => {
                ...
                setting.logs.unshift(
                    new Log({
                        content: `第${playAlarmObj.count}次警告播放:${playAlarmObj.content}`,
                    }),
                );
            })
            .catch((error) => {
                ...
                Msg.closeMsg();
                setting.logs.unshift(
                    new Log({ content: `播放错误:${JSON.stringify(error)}` }),
                );
...
function refreshChart() {
    ...
    chartObj.todayTIRChart.setOption(charOption.value.todayTIRChart, {
        notMerge: true,
        lazyUpdate: true,
    });
    chartObj.todayTTIRChart.setOption(charOption.value.todayTTIRChart, {
        notMerge: true,
        lazyUpdate: true,
    });
    chartObj.myChart.setOption(charOption.value.myChart, {
        notMerge: true,
        lazyUpdate: true,
    });
}
...
function drawLine() {
    ...
    if (!chartObj.todayTIRChart) {
        // 如果不存在，就进行初始化。
        chartObj.todayTIRChart = echarts.init(
            <HTMLElement>todayTIRChart.value,
            "dark",
        );
    }
    if (!chartObj.todayTTIRChart) {
        // 如果不存在，就进行初始化。
        chartObj.todayTTIRChart = echarts.init(
            <HTMLElement>todayTTIRChart.value,
            "dark",
        );
    }
    if (!chartObj.myChart) {
