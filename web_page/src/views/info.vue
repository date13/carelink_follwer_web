    ...
    <MainPanel :no-top="1">
        ...
        <div v-if="!isDialog" class="flex">
            <el-card class="info-card">
                <template #header> Note </template>
                <div class="flex ma-auto w-max items-center">
                    <el-tag class="m-1" effect="dark" type="success">
                        {{ retireDays }}
                    </el-tag>
                    <el-tag class="m-1" effect="dark" type="success">
                        {{ illDays }}
            ...
            <el-card class="info-card">
                ...
                <div class="flex justify-around">
                    <div
                        class="flex flex-col items-center content-center w-1/2"
                    >
                        <el-button
                            :icon="Smoking"
                            circle
        ...
        <div class="flex">
            <el-card class="info-card text-center">
                ...
                <div class="flex flex-col ma-auto w-max">
                    <el-tag class="m-1" effect="dark" type="warning">
                        早
                        <el-input-number
                            v-model="luck.ICR.morning"
                            :max="20"
                            :min="1"
                            :precision="1"
                            :step="0.1"
            ...
            <el-card v-if="!isDialog" class="info-card">
                <template #header> ISF </template>
                <div class="flex flex-col ma-auto w-max">
                    <el-tag class="m-1" effect="dark" type="info">
            ...
            <el-card class="info-card lg">
                ...
                <div class="flex flex-col calc-panel">
                    <el-descriptions :column="3" border>
                        ...
                        <el-descriptions-item>
                            <template #label> 管理员 </template>
                            <el-switch
                                v-model="userSetting.admin"
                                size="small"
                            ></el-switch>
                        </el-descriptions-item>
                        <el-descriptions-item>
                            <template #label> ns_sync </template>
                            <el-switch
                                v-model="userSetting.ns_sync"
                                size="small"
                            ></el-switch>
                        </el-descriptions-item>
                        <el-descriptions-item>
                            <template #label> ns数据源 </template>
                            <el-switch
                                v-model="userSetting.ns"
                                size="small"
                            ></el-switch>
                        </el-descriptions-item>
                        <el-descriptions-item>
                            <template #label> 手动基础 </template>
                            <el-input
                                v-model="userSetting.manual_basal"
                                size="small"
                            ></el-input>
                        </el-descriptions-item>
                        <el-descriptions-item>
                            <template #label> 自动登录 </template>
                            <el-switch
                                v-model="userSetting.auto_login"
                                size="small"
                            ></el-switch>
                        </el-descriptions-item>
                        <el-descriptions-item>
                            <template #label> SSE刷新间隔 </template>
                            {{ userSetting.sse_interval }}
                        </el-descriptions-item>
                        <el-descriptions-item>
                            <template #label> Token刷新间隔 </template>
                            <el-input
                                v-model="
                                    userSetting.carelink_token_refresh_interval
                                "
                                :max="900"
                                :min="60"
                                :step="10"
                        ...
                        <el-descriptions-item>
                            <template #label> Data刷新间隔 </template>
                            <el-input
                                v-model="
                                    userSetting.carelink_data_refresh_interval
                                "
                                :max="120"
                                :min="10"
                                :step="5"
                    ...
                    <el-table :data="jobs" border style="width: 100%">
                        ...
                        >
                            <template #default="{ row }">
                                {{
                                    row.next_run_time
                                        ? dayjs(row.next_run_time).format(
        ...
        <el-card class="info-card lg">
            <template #header>
                <div class="flex justify-between items-center">
                    ...
                    <el-radio-group v-model="curICR" size="small">
                        ...
                        <el-radio-button
                            :value="luck.ICR.afternoon"
                            label="中"
                        />
                        <el-radio-button :value="luck.ICR.evening" label="晚" />
                    </el-radio-group>
                    <el-button size="small" type="primary" @click="addFood"
                        >选择</el-button
                    >
                </div>
            </template>
            <div class="flex flex-col calc-panel">
                <div
                    class="flex justify-between mb-1 pb-1 food-item text-sm font-semibold"
                >
                    <div class="w-10"></div>
                ...
                <div
                    ...
                    class="flex justify-between mb-1 pb-1 food-item text-sm"
                >
                    <div
                        class="w-10 flex justify-start items-center text-red hand"
                        @click="delFood(i)"
                    >
                        <ep-Close></ep-Close>
                    </div>
...
async function loadUserSetting() {
    ...
    if (result) {
        ...
        state.userSetting.manual_basal = parseFloat(
            result.manual_basal,
        ).toFixed(2);
    }
}
...
async function saveSetting() {
    const result = await service.updateUserSetting({
        ...
        carelink_data_refresh_interval: parseInt(
            state.userSetting.carelink_data_refresh_interval,
        ),
        carelink_token_refresh_interval: parseInt(
            state.userSetting.carelink_token_refresh_interval,
        ),
        auto_login: state.userSetting.auto_login,
        ns: state.userSetting.ns,
        manual_basal: state.userSetting.manual_basal
            ? parseFloat(state.userSetting.manual_basal)
            : 0,
        ns_sync: state.userSetting.ns_sync,
    });
    if (result) {
...
const totalCalc = computed(() => {
    ...
    const insulin =
        state.curICR > 0 ? Math.round((carbon / state.curICR) * 100) / 100 : 0;
    return {
        carbon,
        insulin,
    };
