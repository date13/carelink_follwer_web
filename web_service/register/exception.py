...
from core.route_log import get_request_params
from models.const import CROS_HEADER
from models.error import CustomException
...
def response_body(
    ...
    response = {
        ...
        "headers": {  # 解决跨域问题(仿照500错误的响应头)
            "access-router-allow-origin": request.headers.get("origin") or '*',
            "access-router-allow-credentials": "true",
...
def register_exception(app: FastAPI):
    ...
    async def error_user_handler(request: Request, exc: CustomException) -> JSONResponse:
        ...
        if exc.code == status.HTTP_401_UNAUTHORIZED:
            return JSONResponse(status_code=exc.code, headers=CROS_HEADER,
                                content=Result.fail(msg=exc.err_desc).model_dump())
        else:
    ...
    async def validation_exception_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
        ...
        content = Result.fail(msg="请求参数异常!", data=Tools.parseValidation(exc.errors())).model_dump()
        return JSONResponse(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, headers=CROS_HEADER, content=content)
